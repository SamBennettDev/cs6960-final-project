...
def calc_total_mat(board):
    ...
    return {'wp':wp, 'bp':bp, 'wn':wn, 'bn':bn, 'wb':wb, 'bb':bb, 'wr':wr, 'br': br, 'wq':wq, 'bq':bq}, mat_score
...
def calc_indv_score(board):
    ...
    bp_score = sum([-pawntable[chess.square_mirror(i)] for i in board.pieces(chess.PAWN, chess.BLACK)])

    wk_score = sum([knightstable[i] for i in board.pieces(chess.KNIGHT, chess.WHITE)])
    bk_score = sum([-knightstable[chess.square_mirror(i)] for i in board.pieces(chess.KNIGHT, chess.BLACK)])

    wb_score = sum([bishopstable[i] for i in board.pieces(chess.BISHOP, chess.WHITE)])
    bb_score = sum([-bishopstable[chess.square_mirror(i)] for i in board.pieces(chess.BISHOP, chess.BLACK)])

    wr_score = sum([rookstable[i] for i in board.pieces(chess.ROOK, chess.WHITE)])
    br_score = sum([-rookstable[chess.square_mirror(i)] for i in board.pieces(chess.ROOK, chess.BLACK)])

    wq_score = sum([queenstable[i] for i in board.pieces(chess.QUEEN, chess.WHITE)])
    bq_score = sum([-queenstable[chess.square_mirror(i)] for i in board.pieces(chess.QUEEN, chess.BLACK)])

    wki_score = sum([kingstable[i] for i in board.pieces(chess.KING, chess.WHITE)])
    bki_score = sum([-kingstable[chess.square_mirror(i)] for i in board.pieces(chess.KING, chess.BLACK)])

    return {'wp':wp_score, 'bp':bp_score, 'wk':wk_score, 'bk':bk_score, 'wb':wb_score, 'bb':bb_score, 'wr':wr_score, 'br':br_score, 'wq':wq_score, 'bq':bq_score, 'wki':wki_score, 'bki':bki_score}

def calc_advantage(board, indv_scores, mat_score):
